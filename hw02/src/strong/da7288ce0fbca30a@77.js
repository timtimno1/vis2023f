...
function _5(barChartData,constellations,data)
{
  ...
  constellations.forEach((item) => {
    barChartData.push({constellations: item, Gender: "男", count: 0})
    barChartData.push({constellations: item, Gender: "女", count: 0})
  })
  data.forEach((item) => {
...
Plot.plot({
  ...
  marks: [
    Plot.ruleY([0]),
    Plot.barY(barChartData, {x: "constellations", y: "count", fill: "Gender", tip: true}),
  ]
})
)}
